...
func _enter_tree() -> void:
	...
	register_to_id(get_id())


func register_to_id(owner_id: int):
	var global_name: String = ECSComponent.get_script_identifier(get_script())

	var comp_dict: Dictionary = ECSComponent.component_master_dict.get(global_name, {})

	comp_dict[owner_id] = self

	ECSComponent.component_master_dict[global_name] = comp_dict


func get_id() -> int:
	var id: int = ECSComponent.get_id_of_entity(get_parent())
	return id


## This may be called without the need of an instance.
## Example: var player_health: ComponentHealth = ComponentHealth.get_by_id(ECSComponent.get_id_of_entity($Player))
static func get_by_id(script: Script, id: int) -> ECSComponent:
	return component_master_dict.get(ECSComponent.get_script_identifier(script), {}).get(id, null)


static func get_all(script: Script) -> Array[ECSComponent]:
	var output: Array[ECSComponent] = []
	var global_name: String = ECSComponent.get_script_identifier(script)
